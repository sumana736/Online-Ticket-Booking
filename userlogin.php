    ...
    <div class="login-box">
      ...
      <div class="login-box-body">
          ...
          <div class="row">
            <div class="col-xs-12">    
              <div class="checkbox icheck">
                <label>
                  <center><input type="checkbox" name="rem"> Remember Me </center>
                </label>
              </div>                        
            </div><!-- /.col -->
            <div class="col-xs-12">
              <input type="submit" class="btn btn-primary btn-block btn-flat" name="login" value="Log In"> <center>or</center>
			  <a href="" class="btn btn-success btn-block btn-flat" >Create Account</a>
            </div><!-- /.col -->
          </div>
        </form>
		<?php
		if(isset($_POST["login"]))
		{
			$a=$_POST["email"];
			$b=$_POST["password"];
			$selectlogin="select * from user where email='$a' and password='$b'";
			...
			if($rc==1)
			{
				...
				$_SESSION['email']=$a;
				echo"<script>alert('login successfull');window.location.href='userdashboardtrial.php';</script>";
			}
			else
			{
